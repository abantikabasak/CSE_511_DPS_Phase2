...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    val finalresult = spark.sql("select rectangle,count(rectangle) from joinResult group by rectangle order by rectangle")
    val finalresult1 = finalresult.coalesce(1)
    finalresult1.show()
    finalresult1
